...
public class Accounting extends Controller {
  ...
  @Inject
  public Accounting(FormFactory formFactory, MessagesApi messagesApi) {
  ...
  public Result transaction(Integer id, Http.Request request) {
    ...
    return ok(
        views.html.transaction.render(transaction, form, request, mMessagesApi.preferred(request)));
  }
  ...
  public Result makeNewTransaction(Http.Request request) {
    ...
    if (filledForm.hasErrors()) {
      ...
      return badRequest(
          create_transaction.render(filledForm, request, mMessagesApi.preferred(request)));
    } else {
      try {
        Transaction transaction =
            Transaction.create(
                filledForm, Utils.getOrg(request), Application.getCurrentUser(request));
    ...
    try {
      ...
      Transaction transaction =
          Transaction.findById(
              Integer.parseInt(form.field("id").value().get()), Utils.getOrg(request));
  ...
  public Result balances(Http.Request request) {
    ...
    return ok(
        balances.render(
            personalAccounts, nonPersonalAccounts, request, mMessagesApi.preferred(request)));
  }
  ...
  public Result bankCashBalance(Http.Request request) {
    applyMonthlyCredits(Utils.getOrg(request));
    return ok(
        bank_cash_balance.render(
            TransactionList.allCash(Utils.getOrg(request)),
            request,
  ...
  public Result transactionsReport(Http.Request request) {
    applyMonthlyCredits(Utils.getOrg(request));
    return ok(
        transactions_report.render(
            TransactionList.blank(), request, mMessagesApi.preferred(request)));
  }

  @Secured.Auth(UserRole.ROLE_ACCOUNTING)
  public Result runTransactionsReport(Http.Request request) throws IOException {
    Form<TransactionList> form =
        mFormFactory.form(TransactionList.class).withDirectFieldAccess(true);
  ...
  public Result report(Http.Request request) {
    applyMonthlyCredits(Utils.getOrg(request));
    return ok(
        accounting_report.render(new AccountingReport(), request, mMessagesApi.preferred(request)));
  }

  @Secured.Auth(UserRole.ROLE_ACCOUNTING)
  public Result runReport(Http.Request request) {
    Form<AccountingReport> form =
        mFormFactory.form(AccountingReport.class).withDirectFieldAccess(true);
    Form<AccountingReport> filledForm = form.bindFromRequest(request);
    if (filledForm.hasErrors()) {
      System.out.println("ERRORS: " + filledForm.errorsAsJson().toString());
      return badRequest(
          accounting_report.render(
              new AccountingReport(), request, mMessagesApi.preferred(request)));
    }
  ...
  public Result editAccount(Integer id, Http.Request request) {
    ...
    return ok(
        edit_account.render(form, account.getIsActive(), request, mMessagesApi.preferred(request)));
  }
  ...
  public Result saveAccount(Http.Request request) {
    ...
    Account account =
        Account.findById(Integer.parseInt(form.field("id").value().get()), Utils.getOrg(request));
  ...
  static void applyMonthlyCredits(Organization org) {
    ...
    List<Account> accounts =
        Account.allWithMonthlyCredits(org).stream()
            .filter(
                a ->
                    a.getDateLastMonthlyCredit() == null
                        || a.getDateLastMonthlyCredit().before(monthStartDate))
