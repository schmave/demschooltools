...
public class Secured {
   ...
   *
   * <p>The username is retrieved from the session cookie, and added to the HTTP request's <code>
  ...
  public static class Authenticator extends Results {
    ...
    public String getUsernameOrIP(final Http.Request request, boolean allow_ip) {
      sLogger.debug(
          "Authenticator::getUsername " + request.host() + " " + request + ", " + allow_ip);
      ...
      if (u != null) {
        ...
        if (the_user != null) {
          ...
          if (!the_user.getActive()) {
            return unauthorized(
                "Your account with email address " + the_user.getEmail() + " is inactive.");
          }
        }
      }
