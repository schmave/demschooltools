...
public class Settings extends Controller {
  ...
  @Inject
  public Settings(final FormFactory ff, MessagesApi messagesApi) {
    mFormFactory = ff;
  ...
  public Result viewSettings(Http.Request request) {
    ...
    return ok(
        view_settings.render(
            ...
            Public.sConfig.getConfig("school_crm"),
            request,
  ...
  public Result editNotifications(Http.Request request) {
    ...
    if (values.containsKey("remove_notification_id")) {
      NotificationRule.findById(
              Integer.parseInt(values.get("remove_notification_id")[0]), Utils.getOrg(request))
          .delete();
    } else {
      ...
      if (!email.matches("^\\S+@\\S+.\\S+$")) {
        return redirect(routes.Settings.viewSettings())
            .flashing(
                ...
                "'"
                    + email
                    + "' does not seem to be a valid email address. Notification not created.");
      }
      ...
      if (values.containsKey("school_meeting")) {
        NotificationRule.create(
            NotificationRule.TYPE_SCHOOL_MEETING, null, email, Utils.getOrg(request));
      }
    }
  ...
  public Result viewTaskLists(Http.Request request) {
    ...
    return ok(
        view_task_lists.render(
            TaskList.allForOrg(Utils.getOrg(request)),
            list_form,
            request,
            mMessagesApi.preferred(request)));
  }
  ...
  public Result viewTaskList(Integer id, Http.Request request) {
    ...
    return ok(
        settings_task_list.render(
            list, list_form.fill(list), task_form, request, mMessagesApi.preferred(request)));
  }
  ...
  public Result newTaskList(Http.Request request) {
    ...
    if (form_data.get("tag_id") == null) {
      return redirect(routes.Settings.viewTaskLists())
          .flashing("error", "No tag specified for checklist. No checklist was created.");
    }
  ...
  public Result saveTaskList(Http.Request request) {
    ...
    if (form_data.containsKey("tag_id")) {
      list.setTag(
          Tag.findById(Integer.parseInt(form_data.get("tag_id")[0]), Utils.getOrg(request)));
    }
  ...
  public Result viewAccess(Http.Request request) {
    ...
    List<User> users =
        User.find.query().where().eq("organization", org).order("name ASC").findList();
    ...
    return ok(
        view_access.render(
            users_to_show, allowed_ip, user_form, request, mMessagesApi.preferred(request)));
  }
    ...
    if (existing_user != null) {
      if (existing_user.getOrganization().getId().equals(org.getId())) {
        if (existing_user.getName().equals(User.DUMMY_USERNAME)) {
          ...
          return redirect(routes.Settings.viewAccess())
              .flashing("error", "That email address (" + email + ") already has an account.");
        }
      } else {
        return redirect(routes.Settings.viewAccess())
            .flashing(
                ...
                "That email address ("
                    + email
                    + ") already has an account for another school. "
