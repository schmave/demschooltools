...
public class CRM extends Controller {
        ...
        cached_page.render(
            new CachedPage(
              ...
              String render() {
                ...
                return people_index
                    .render(recent_comments, request, mMessagesApi.preferred(request))
                    .toString();
              }
            },
            request,
  ...
  public Result person(Integer id, Http.Request request) {
    ...
    List<Person> family_members =
        Person.find
            ...
            .where()
            .isNotNull("family")
            .eq("family", the_person.getFamily())
            .ne("person_id", the_person.getPersonId())
            .findList();
    ...
    List<Comment> all_comments =
        Comment.find.query().where().in("person_id", family_ids).order("created DESC").findList();
  ...
  public Result jsonPeople(String query, Http.Request request) {
    ...
    for (String term : query.split(" ")) {
      ...
      Expression this_expr =
          Expr.or(
              Expr.ilike("lastName", "%" + term + "%"), Expr.ilike("firstName", "%" + term + "%"));
      this_expr = Expr.or(this_expr, Expr.ilike("address", "%" + term + "%"));
      this_expr = Expr.or(this_expr, Expr.ilike("email", "%" + term + "%"));
      this_expr = Expr.or(this_expr, Expr.ilike("displayName", "%" + term + "%"));
  ...
  public Collection<Person> getTagMembers(Integer tagId, String familyMode, Organization org) {
    ...
    if (familyMode.equals("family_no_kids")) {
      ...
      for (Person p2 : selected_people) {
        if (p2.getDob() == null || p2.calcAge() > 18) {
          no_kids.add(p2);
        }
      }
  ...
  public Result allPeople(Http.Request request) {
    return ok(
        all_people.render(
            Person.all(Utils.getOrg(request)), request, mMessagesApi.preferred(request)));
  }
  ...
  public Result editTag(Integer id, Http.Request request) {
    Form<Tag> filled_form =
        mFormFactory.form(Tag.class).fill(Tag.findById(id, Utils.getOrg(request)));
  ...
  public Result removePeopleFromTag(Http.Request request) {
    ...
    for (String key_name : values.keySet()) {
      if (key_name.startsWith(prefix) && values.get(key_name)[0].equals("on")) {
        Person p =
            Person.findById(
                Integer.parseInt(key_name.substring(prefix.length())), Utils.getOrg(request));
  ...
  public Result downloadTag(Integer id, Http.Request request) throws IOException {
    ...
    return ok(baos.toByteArray())
        ...
        .withHeader("Content-Disposition", "attachment; filename=" + the_tag.getTitle() + ".xlsx");
  }
  ...
  public Result makeNewPerson(Http.Request request) {
    ...
    if (filledForm.hasErrors()) {
      ...
      return badRequest(new_person.render(filledForm, request, mMessagesApi.preferred(request)));
    } else {
  ...
  public Result savePersonEdits(Http.Request request) {
    ...
    if (filledForm.hasErrors()) {
      ...
      return badRequest(edit_person.render(filledForm, request, mMessagesApi.preferred(request)));
    }

    Person updatedPerson = Person.updateFromForm(filledForm, Utils.getOrg(request));
    return redirect(
        routes.CRM.person(
            (updatedPerson.getIsFamily() ? updatedPerson.family_members.get(0) : updatedPerson)
                .getPersonId()));
  }
  ...
  public Result addComment(Http.Request request) {
    ...
    new_comment.setPerson(
        Person.findById(
            Integer.parseInt(filledForm.field("person").value().get()), Utils.getOrg(request)));
    ...
    if (task_id_string.length() > 0 || new_comment.getMessage().length() > 0) {
      ...
      if (filledForm.field("send_email").value().isPresent()) {
        for (NotificationRule rule :
          ...
          play.libs.mailer.Email mail = new play.libs.mailer.Email();
          mail.setSubject(
              "DemSchoolTools comment: "
                  + new_comment.getUser().getName()
                  + " & "
                  + new_comment.getPerson().getInitials());
          mail.addTo(rule.getEmail());
          mail.setFrom("DemSchoolTools <noreply@demschooltools.com>");
          mail.setBodyHtml(
              comment_email
                  .render(
                      Comment.find.byId(new_comment.getId()),
                      request,
                      mMessagesApi.preferred(request))
                  .toString());
          sMailer.send(mail);
  ...
  public static int calcAgeAtBeginningOfSchool(Person p) {
    ...
    return (int)
        ((ModelUtils.getStartOfYear().getTime() - p.getDob().getTime())
            / 1000
            / 60
            / 60
            / 24
