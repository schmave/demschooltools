...
def add_org_config_to_file(filename, template_name):
    ...
    if filename.endswith(".java"):
        ...
        if (
            "orgConfig" in existing_content
            and "orgConfig: OrgConfig" not in existing_content
        ):
        ...
        for i, line in enumerate(lines):
            if search_string in line and "orgConfig" not in line:
                needs_save = True
                lines[i] = line.replace(search_string, f"{search_string}orgConfig, ")
...
def add_current_user_to_file(filename, template_name):
    ...
    if filename.endswith(".java"):
        ...
        if (
            "currentUsername" in existing_content
            and "currentUsername: Optional[String]" not in existing_content
        ):
