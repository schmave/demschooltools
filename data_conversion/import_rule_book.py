...
for line in open(sys.argv[1], "r"):
    ...
    if chapter_match or rule_match:
        if rule_num and rule_title and rule_content:
            cur.execute(
                "INSERT into entry(num, title, section_id, content) VALUES (%s, %s, %s, %s)",
    ...
    if chapter_match:
        cur.execute(
            "INSERT into chapter (num, title) VALUES (%s, %s) returning id",
            (chapter_match.group(1) + "0", chapter_match.group(2)),
        )
        chapter_id = int(cur.fetchone()[0])

        cur.execute(
            "INSERT into section(num, title, chapter_id) VALUES (%s, %s, %s) returning id",
