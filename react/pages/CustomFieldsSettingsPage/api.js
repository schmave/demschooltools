...
export const fetchGroups = (baseUrl, entityType) => {
  const url = `${ensureTrailingSlash(baseUrl)}?entity_type=${encodeURIComponent(entityType)}`;
  return jsonFetch(url);
};

export const createGroup = (baseUrl, payload) =>
  jsonFetch(ensureTrailingSlash(baseUrl), {
    method: 'POST',
    body: payload,
  });

export const updateGroup = (baseUrl, id, payload) =>
  jsonFetch(buildDetailUrl(baseUrl, id), {
    method: 'PUT',
    body: payload,
  });

export const patchGroup = (baseUrl, id, payload) =>
  jsonFetch(buildDetailUrl(baseUrl, id), {
    method: 'PATCH',
    body: payload,
  });

export const deleteGroup = async (baseUrl, id) => {
  const response = await fetch(buildDetailUrl(baseUrl, id), {
    method: 'DELETE',
    credentials: 'same-origin',
    headers: {
