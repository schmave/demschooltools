...
public abstract class ExternalAuthProvider extends AuthProvider {
  ...
  private abstract static class SettingKeys {
    @Deprecated public static final String SECURE_REDIRECT_URI = "secureRedirectUri";
  ...
  protected static String generateURI(
      final String location, final List<? extends NameValuePair> params) {
    final HttpGet m = new HttpGet(location + "?" + URLEncodedUtils.format(params, "UTF-8"));
  ...
  private boolean useSecureRedirectUri() {
    Boolean secure = getConfiguration().getBoolean(SettingKeys.REDIRECT_URI_SECURE);
    if (secure == null) {
      // only for backwards compatibility
      secure = getConfiguration().getBoolean(SettingKeys.SECURE_REDIRECT_URI);
    }
    return secure != null ? secure : false;
  }

  protected String getRedirectUrl(final Request request, final List<? extends NameValuePair> params)
  ...
  protected String getRedirectUrl(final Request request) throws ResolverMissingException {
    ...
    if (getConfiguration().hasPath(SettingKeys.REDIRECT_URI_HOST)
        && !(overrideHost = getConfiguration().getString(SettingKeys.REDIRECT_URI_HOST))
            .trim()
            .isEmpty()) {
