...
public class FacebookAuthProvider extends OAuth2AuthProvider<FacebookAuthUser, FacebookAuthInfo> {
  ...
  public abstract static class SettingKeys extends OAuth2AuthProvider.SettingKeys {
    public static final String DISPLAY = "display";
  }

  public abstract static class FacebookConstants extends OAuth2AuthProvider.Constants {
    public static final String DISPLAY = "display";
  }
  ...
  protected FacebookAuthUser transform(FacebookAuthInfo info, final String state)
    ...
    final String fields = getConfiguration().getString(USER_INFO_FIELDS_SETTING_KEY);
    ...
    if (r.getStatus() >= 400) {
      ...
      if (respJson.size() < 2) {
        throw new AccessTokenException(
            "At least two values were expected, but got " + respJson.size());
      }
  ...
  protected List<NameValuePair> getAuthParams(
    ...
    if (c.hasPath(SettingKeys.DISPLAY)) {
      params.add(
          new BasicNameValuePair(FacebookConstants.DISPLAY, c.getString(SettingKeys.DISPLAY)));
    }
