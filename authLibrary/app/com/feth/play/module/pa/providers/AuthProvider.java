...
public abstract class AuthProvider {
  ...
  public abstract static class Registry {
    ...
    public static void register(final String provider, final AuthProvider p) {
      ...
      if (previous != null) {
        sLogger.warn("There are multiple AuthProviders registered for key '" + provider + "'");
      }
    }
      ...
      for (final String key : neededSettings) {
        if (!c.hasPath(key) || c.getString(key).isEmpty()) {
          throw new RuntimeException(
              "Provider '" + getKey() + "' missing needed setting '" + key + "'");
        }
      }
    }
   ...
   */
  public abstract Object authenticate(final Request request, final Object payload)
      throws AuthException;
