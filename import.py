...
for line in csv.DictReader(open(sys.argv[1], 'rb')):
	...
	ors = ([cur.mogrify("first_name like %s", [n]) for n in name_splits] + 
		[cur.mogrify("last_name like %s",[n]) for n in name_splits])
	ors = " OR ".join(ors)
	...
	cur.execute("UPDATE person set email='%s', address='%s', city='%s', state='%s', zip='%s' WHERE person_id=%d" % (
		line["Email"], line["Address 1"], line["City"], line["State"], line["Zip"], id))
