...
module.exports = class Classes extends React.Component {
  state = {
    classes: classStore.getClasses(true),
    filterText: "",
  ...
  setupState = (classes, students) => {
    ...
    if (selectedClass && selectedClass._id) {
      var id = selectedClass._id,
        matching = classes.filter(function (cls) {
          return cls._id === id;
        });
  ...
  getStudentRowsInCurrentClass = () => {
    var a = 1;
    var t = this.filterStudents(
      this.state.selectedClass.students.sort(this.rankAlphabetically),
    ).map(
      function (stu) {
        return (
          <div
            key={"t" + this.state.selectedClass._id + "-" + stu.student_id}
            className="in-class panel panel-info student-listing col-sm-4"
          >
            <div>
              <div className="name"> {stu.name} </div>
              <div className="attendance-button">
                <button
                  onClick={this.deleteFromClass.bind(this, stu)}
  ...
  getStudentRowsNotInCurrentClass = () => {
    var filtered = this.state.students
      .sort(this.rankAlphabetically)
      .filter(this.selectedStudentContains);
    var t = this.filterStudents(filtered).map(
      function (stu) {
        return (
          <div
            key={"NOTCLASS-" + stu._id}
            className="out-class panel panel-info student-listing col-sm-11"
          >
            <div>
              <div className="attendance-button">
                <button
                  id={"add-" + stu._id}
                  onClick={this.addToClass.bind(this, stu)}
                  className="btn btn-xs btn-primary"
                >
  ...
  makeActivateClassButtonAndPanel = (selectedClass) => {
    ...
    if (selectedClass.active) {
      ...
      activateClassButton = (
        <button
          onClick={this.activateClass.bind(this, selectedClass._id)}
          ...
          <div className="col-sm-10 column">
            <div className={"panel " + panel}>
              <div className="panel-heading absent">
                <b> {this.state.selectedClass.name} </b>

                <button
                  className="btn btn-primary btn-sm"
                  onClick={this.toggleEdit.bind(this, this.state.selectedClass)}
                >
                  Edit
                </button>
              </div>
